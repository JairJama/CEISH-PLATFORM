...
export class WorkflowEventInterceptor implements NestInterceptor {
        ...
        if (entityType && entityId) {
          await this.prisma.workflowEvent.create({
            data: {
              ...
              previousStatus,
              newStatus,
              description: `${user.role ?? user.userType ?? "user"} performed ${method} on ${entityType}`,
              metadata: { method, url },
            },
          });
        }
  ...
  private extractPreviousStatus(body: Record<string, unknown>): string | null {
    const data = this.toRecord(body.data);
    return (
      this.toStringOrNull(body.previousStatus) ??
      this.toStringOrNull(data?.previousStatus)
    );
  }
  ...
  private extractNewStatus(body: Record<string, unknown>): string | null {
    const data = this.toRecord(body.data);
    return (
      this.toStringOrNull(body.status) ?? this.toStringOrNull(data?.status)
    );
  }
